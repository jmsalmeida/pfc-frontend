...
import { Divider, Layout, Icon, TopNavigation, TopNavigationAction, Text, Input, Radio, Button } from '@ui-kitten/components';
...
export const RegisterPartyPlaceScreen = ({ navigation }) => {
  ...
  const registerPartyPlace = (event) => {
    ...
    const partyPlace = {
      ...
      city, main_contact, cellphone , email, emailConfirmation,
      password, passwordConfirmation
    };

    api.post('/party-places', { body: JSON.stringify(partyPlace) })
    .then((response) => {
      if (response.ok) {
        Toast.show({
          type: 'success',
      ...
      <Layout style={styles.container}>
        <ScrollView>
          <Layout style={styles.formContainer}>
            ...
            <Input style={styles.inputContainer}
              label='CNPJ'
              caption={renderCaption((cnpjPartyPlace ? !cnpj.isValid(cnpjPartyPlace) : false), "CNPJ inválido!")}
              required={true}
          ...
          <Layout style={styles.formContainer}>
            ...
            <Layout style={styles.inputContainerRow}>
              <Input style={{flex: 1, marginRight: 5}}
                label='Número'
                required={true}
                placeholder='Número'
                value={number}
              ...
              <Input style={{flex: 3, marginLeft: 5}}
                label='Bairro'
                required={true}
          ...
          <Layout style={styles.formContainer}>
            ...
            <Layout style={styles.inputContainerRow}>
              <Input style={{flex: 2, marginRight: 5}}
                label='Telefone'
                placeholder='9999-9999'
                value={phone}
                onChangeText={nextValue => setPhonePlace(nextValue)}
              />
              <Input style={{flex: 2, marginLeft: 5}}
                label='Celular'
                required={true}
          ...
          <Layout style={styles.formContainer}>
            ...
            <Input style={styles.inputContainer}
              ...
              caption={renderCaption(invalidConfirmation(email.toLowerCase(), emailConfirmation.toLowerCase()), "Os e-emails devem ser iguais!")}
            />
            ...
            <Input style={styles.inputContainer}
              ...
              caption={renderCaption(invalidConfirmation(email.toLowerCase(), emailConfirmation.toLowerCase()), "Os e-mails devem ser iguais!")}
            />
            ...
            <Layout style={styles.inputContainerRow}>
              <Input style={{flex: 2, marginRight: 5}}
                ...
                caption={renderCaption(invalidConfirmation(password, passwordConfirmation), "As senhas devem ser iguais!")}
              />
        ...
        <Layout style={styles.submitButton}>
          <Button
            disabled={disableButton([
              name, cnpjPartyPlace, postal_code, street, number,
              district, city, main_contact, cellphone, email, emailConfirmation,
              (email.toLowerCase() === emailConfirmation.toLowerCase()),
              password, passwordConfirmation, (password === passwordConfirmation), checked, cnpj.isValid(cnpjPartyPlace)
            ])}
